...
void buat_hanning(data_32 *out, int M)
{
    ...
    for (i=0; i<= M; i++)
    {
        out[i] = (data_32) (0.5 - (0.5 * cos ( 2 * M_PI * i / M )));
    }

    //skala_window(out, M, faktor_hanning );
}
...
void buat_hamming(data_32 *out, int M)
{
    ...
    for (i=0; i<= M; i++)
    {
        out[i] = (data_32) (0.54 - (0.46 * cos ( 2 * M_PI * i / M )));
    }

    //skala_window(out, M, faktor_hamming );
}
...
void buat_blackman(data_32 *out, int M)
{
    ...
    for (i=0; i<= M; i++)
    {
        out[i] = (data_32)  (0.42 - (0.5 * cos ( 2 * M_PI * i / M )) + (0.08 * cos (4 * M_PI * i / M)));
    }

    //skala_window(out, M, 1.0 );
}
...
#if 0
int buat_bandpass(data_32 *filter_kernel, int M, data_32 fc_L, data_32 fc_H, unsigned short kind)
{
